...
- Sidebar network chords (`Alt+Left` / `Alt+Right`, collapse/expand) work from
  the composer on macOS instead of moving the caret by word. The shortcuts
  sheet and Connect hint show `Cmd` and `Option` in place of `Ctrl` and `Alt`.
