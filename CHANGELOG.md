...
- Native macOS build and CI artifact (`omairc-*-macos-arm64.zip`). `bin/build-macos` and `bin/package-macos` produce a Developer ID signed, notarized `.app` zip on GitHub Actions when those secrets are present; version tags refuse to ship an ad-hoc bundle. Local Homebrew-Qt builds stay ad-hoc.
