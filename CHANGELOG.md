...
- The About update checks click the status line after it can accept a click,
  and the half-page transcript scroll test no longer times out the package
  check.
...
- Page Up / Page Down (and Shift variants) page the member nick list when it
...
- An in-source build no longer hides the generated version header. A later
  shadow build still reports `OMAIRC_BUILD_VERSION` from `omairc --version`,
  so the Arch package check can see the override.
...
- Native macOS build and CI artifacts (`omairc-*-macos-arm64.zip` and `omairc-*-macos-x64.zip`). `bin/build-macos` Developer ID signs when `CODESIGN_IDENTITY` is set. GitHub Actions notarizes and staples on version tags; pull-request zips stay signed but not notarized because Apple can hold a new Developer ID In Progress for hours. Tag releases refuse to ship an ad-hoc bundle. Local Homebrew-Qt builds stay ad-hoc.
