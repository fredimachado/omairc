...
- IRCv3 `sasl`, `sts`, `echo-message`, `server-time`, `multi-prefix`, `away-notify`, `draft/metadata-2`, `message-tags`, `chghost`, `cap-notify`, `batch`, `chathistory`, and `znc.in/playback`. Reconnects with backoff.
...
Each version tag also publishes `omairc-*-macos-arm64.zip` (Apple Silicon CI; Intel builds use `macos-x64`) on the [GitHub release](https://github.com/fredimachado/omairc/releases/latest). Pull-request CI uploads the same zip as a workflow artifact. CI signs the bundle with Developer ID, notarizes it, and staples the ticket, so Gatekeeper should accept a release zip. Local Homebrew-Qt builds stay ad-hoc.
