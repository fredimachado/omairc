...
- After a profile exists, click the network name in the sidebar header to open it again.

## Driving it with control-omairc

Preconditions:

- A fresh compiled launch (`control-omairc launch`) is titled ` - Omairc` and shows Connect. That is the desktop entry point.
...
- **First-run sheet.** After an isolated launch, run `control-omairc title` then `control-omairc screenshot --feature connect --name first-run`. The title is ` - Omairc`. The overlay heading is `Connect`. Host is `irc.libera.chat`, port is `6697`, TLS is on, Autojoin is `#omarchy`, Nick is empty, the problem line is `Nick is required`, and Apply is muted.
