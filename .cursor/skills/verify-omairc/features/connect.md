...
- `connect-keyboard` tabs from the network list through the form to Apply, and Shift+Tab returns. Enter walks the next field. Ctrl+Enter Applies the selected network from any focus, including Preferences. Ctrl+Tab / Ctrl+Shift+Tab switch Connect tabs. Alt+Left / Alt+Right walk networks from anywhere in the sheet (focus stays in a field unless it is already in the rail). Ctrl+N adds a network and focuses Name. Ctrl+Shift+Delete arms, then removes, the selected network. Enter from Host with an empty nick keeps `Nick is required` and does not start a session. Apply, Discard, and Add network are reachable without a mouse.
...
- The Preferences tab is global. `Reopen direct messages on startup` and `Open conversations at unread` write immediately and do not use Apply. Enter on those toggles walks focus the same way it does on Connection switches. Ctrl+Enter applies the selected network from this tab as well.
