...
- `connect-modal` covers the whole window. Sidebar servers, conversations, and channel members cannot be clicked or walked while Connect is open. After a profile exists, a click on that dimmer still dismisses the sheet without changing the selected conversation.
