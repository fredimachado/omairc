...
- The first `import` after a fresh `launch` can grab a blank frame. `composer` does not wait. This recipe sends Escape first. Escape is disabled on a conversation, so `#omarchy` stays open.
