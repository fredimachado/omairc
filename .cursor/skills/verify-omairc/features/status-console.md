...
- **Toggle with no conversation.** Run `control-omairc key --key ctrl+shift+s`. The title stays `irc.libera.chat Status`. Status cannot close while nothing is selected.
...
- `Ctrl+Shift+S` does not close Status when no conversation is selected. First-run Connect is that state.
