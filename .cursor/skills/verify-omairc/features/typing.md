...
- **Member glyph.** Send Escape so the window has painted, then capture the member panel. Run `control-omairc key --key Escape` then `control-omairc screenshot --feature typing --name member-glyph`. Three dots sit beside `&anna`. The other rows do not.
