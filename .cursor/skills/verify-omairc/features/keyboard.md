...
- `keyboard-scroll` pages the visible transcript with `Page Up` / `Page Down` while the composer stays focused. `Shift+Page Up` / `Shift+Page Down` hop about half as far.
...
- Press `Page Up` / `Page Down` to scroll without leaving the composer. `Shift+Page Up` / `Shift+Page Down` hop about half a page.
...
- Page Up / Page Down and Shift+Page Up / Shift+Page Down are disabled while Connect is visible.
