...
- `keyboard-link` opens a filter overlay with `Ctrl+Shift+O`. It lists `http`/`https` URLs from the current transcript (conversation or Status), newest first. Type filters the list. Up/Down wrap and reveal the source row. Enter opens the URL or joins an INVITE channel. Escape dismisses and returns to the composer without changing the draft. It is a no-op while Connect is visible. While open, `Alt+Down` does not walk conversations and `Ctrl+/` does not open the shortcuts sheet on top.
...
- `keyboard-sheet` toggles the shortcut list with `Ctrl+/`. Escape closes the sheet before Status. The list includes `Ctrl+W`, `Ctrl+C`, `Ctrl+F`, `Ctrl+K`, `Ctrl+Shift+O`, `Ctrl+Shift+K`, `Ctrl+Shift+A`, `Ctrl+Shift+S`, per-network collapse/expand/move, and collapse-all / expand-all. `Ctrl+Q` still quits the process.
...
- Press `Ctrl+Shift+O` to open the link sheet for the current transcript.
...
compare --before test-artifacts/verify/keyboard/after-close.png --after test-artifacts/verify/keyboard/server-list-collapsed.png
key --key ctrl+shift+s
screenshot --feature keyboard --name server-list-restored
compare --before test-artifacts/verify/keyboard/server-list-collapsed.png --after test-artifacts/verify/keyboard/server-list-restored.png
```
...
- **Server list.** Press `Ctrl+Shift+S`. Run `control-omairc key --key ctrl+shift+s` then `control-omairc screenshot --feature keyboard --name server-list-collapsed`. The left column is gone and the transcript is full width. Run `control-omairc compare --before test-artifacts/verify/keyboard/after-close.png --after test-artifacts/verify/keyboard/server-list-collapsed.png`. Press `Ctrl+Shift+S` again. Run `control-omairc key --key ctrl+shift+s` then `control-omairc screenshot --feature keyboard --name server-list-restored`. The column returns with the networks and the identity footer. Run `control-omairc compare --before test-artifacts/verify/keyboard/server-list-collapsed.png --after test-artifacts/verify/keyboard/server-list-restored.png`. Both compares must report a pixel change. This is independent of per-network collapse.
...
- `Ctrl+Shift+O` is disabled while Connect is visible, the same rule as `Ctrl+K`. It stays enabled while the link sheet is already open so the chord can toggle it closed.
