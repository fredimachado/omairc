...
- **Shortcut sheet on first run.** After `control-omairc launch`, run `control-omairc key --key ctrl+slash` then `control-omairc screenshot --feature keyboard --name after-ctrl-slash`. The sheet is a grouped cheat sheet under MOVE, JUMP, WRITE, CONNECT, and WINDOW headings. Opposite network chords share a row (`Alt+Shift+Left / Right` collapse / expand network, `Ctrl+Alt+Shift+Left / Right` collapse / expand all, `Alt+Shift+Up / Down` move network). WRITE lists `Page Up` / `Page Down` scroll, `Shift+Page Up` / `Shift+Page Down` scroll half page, and `Ctrl+Home` / `Ctrl+End` top / bottom. CONNECT lists `Ctrl+,` and the Connect-sheet chords (`Ctrl+Tab`, `Ctrl+N`, `Ctrl+Shift+Delete`, `Ctrl+Enter`) so add-network is not read as global. WINDOW lists `Ctrl+Shift+S` server list alongside the members column. The list still includes walk conversations, walk networks, jump to conversation, jump to nick, unread, Status, members panel, focus members, close direct message, composer, find, send, nick complete, history, Escape, this sheet, `/disconnect`, and quit. If the first send does not open the sheet, send `ctrl+slash` again. Run `control-omairc key --key Escape`. Connect stays open.
