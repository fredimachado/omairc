...
- `keyboard-sheet` toggles the shortcut list with `Ctrl+/`. Escape closes the sheet before Status. The list includes `Ctrl+W`, `Ctrl+F`, `Ctrl+K`, `Ctrl+Shift+K`, `Ctrl+Shift+A`, `Ctrl+Shift+S`, per-network collapse/expand/move, collapse-all / expand-all, and `/disconnect`. `Ctrl+Q` still quits the process.
