...
- `slash-live-dispatch` sends `/me`, `/join`, `/part`, `/nick`, `/disconnect` (`/quit`), `/clear`, `/close`, `/query`, `/msg`, `/topic`, `/notice`, `/away`, `/back`, `/whois`, `/mode`, `/kick`, `/ignore`, `/unignore`, `/ignored`, `/op`, `/deop`, `/voice`, `/devoice`, `/ban`, `/invite`, `/ns`, `/cs`, `/raw` (`/quote`), and `/help` when `irc` is bound. `/disconnect` and `/quit` idle the focused network. `/q` is unknown. `/msg` and `/ns`/`/cs` send without opening or selecting a DM. `/join` also matches `/j`, accepts comma-separated channels, and supports an optional key after whitespace. Empty `/join` joins the latest inbound invite. Clicking the channel token on a Status `INVITE` line joins that channel. `/part` also matches `/leave`. `/ignore` hides private messages, notices, and invites from that nick. Channel text stays visible. `/ignored` lists the nicks on Status. `/help` prints the catalog in the active conversation. A Status `/help` stays on Status.
