...
- `slash-live-dispatch` sends `/me`, `/join`, `/part`, `/nick`, `/quit`, `/clear`, `/close`, `/query` (`/msg`), `/topic`, `/notice`, `/away`, `/back`, `/whois`, and `/mode` when `irc` is bound.
...
- **Live catalog.** Do not claim `/away`, `/whois`, `/topic`, `/query`, `/notice`, `/part`, or `/close` on first-run Connect. It is `verified-unreachable` until a session has completed Connect. The attempted compiled route is `control-omairc launch` (title `irc.libera.chat Status`, Apply would start a real network).
